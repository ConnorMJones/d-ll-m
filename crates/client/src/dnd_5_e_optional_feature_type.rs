// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

use super::optional_feature_prereq_type::OptionalFeaturePrereq;
use super::optional_feature_type_type::OptionalFeatureType;

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct Dnd5EOptionalFeature {
    pub id: u64,
    pub name: String,
    pub source: String,
    pub feature_types: Vec<OptionalFeatureType>,
    pub prerequisite: Option<OptionalFeaturePrereq>,
    pub description: String,
}

impl __sdk::InModule for Dnd5EOptionalFeature {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `Dnd5EOptionalFeature`.
///
/// Provides typed access to columns for query building.
pub struct Dnd5EOptionalFeatureCols {
    pub id: __sdk::__query_builder::Col<Dnd5EOptionalFeature, u64>,
    pub name: __sdk::__query_builder::Col<Dnd5EOptionalFeature, String>,
    pub source: __sdk::__query_builder::Col<Dnd5EOptionalFeature, String>,
    pub feature_types: __sdk::__query_builder::Col<Dnd5EOptionalFeature, Vec<OptionalFeatureType>>,
    pub prerequisite:
        __sdk::__query_builder::Col<Dnd5EOptionalFeature, Option<OptionalFeaturePrereq>>,
    pub description: __sdk::__query_builder::Col<Dnd5EOptionalFeature, String>,
}

impl __sdk::__query_builder::HasCols for Dnd5EOptionalFeature {
    type Cols = Dnd5EOptionalFeatureCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        Dnd5EOptionalFeatureCols {
            id: __sdk::__query_builder::Col::new(table_name, "id"),
            name: __sdk::__query_builder::Col::new(table_name, "name"),
            source: __sdk::__query_builder::Col::new(table_name, "source"),
            feature_types: __sdk::__query_builder::Col::new(table_name, "feature_types"),
            prerequisite: __sdk::__query_builder::Col::new(table_name, "prerequisite"),
            description: __sdk::__query_builder::Col::new(table_name, "description"),
        }
    }
}

/// Indexed column accessor struct for the table `Dnd5EOptionalFeature`.
///
/// Provides typed access to indexed columns for query building.
pub struct Dnd5EOptionalFeatureIxCols {
    pub id: __sdk::__query_builder::IxCol<Dnd5EOptionalFeature, u64>,
}

impl __sdk::__query_builder::HasIxCols for Dnd5EOptionalFeature {
    type IxCols = Dnd5EOptionalFeatureIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        Dnd5EOptionalFeatureIxCols {
            id: __sdk::__query_builder::IxCol::new(table_name, "id"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for Dnd5EOptionalFeature {}
